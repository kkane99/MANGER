...
from modules.layer.rnd import RND
import pdb
...
class SEPQLearner:
    def __init__(self, mac, scheme, logger, args):
        ...
        self.rnd_optimiser = Adam(params=self.rnd.parameters(),  lr=args.rnd_lr, weight_decay=getattr(args, "weight_decay", 0))
        ...
        if self.args.optimizer == 'adam':
            ...
            self.optimiser = RMSprop(params=self.params, lr=args.lr, alpha=0.9, eps=args.optim_eps)
    ...
    def train(self, batch: EpisodeBatch, t_env: int, episode_num: int, per_weight=None):
        ...
        # uptdate every 2 times
        rnd_loss = self.rnd(batch["obs"])
        if self.rnd.update_freq >= 1:
            rnd_loss.mean().backward()
            self.rnd.update_freq = 0
        else:
            self.rnd.update_freq += 1
        # print("RND LOSS:",rnd_loss.mean())
        self.rnd_optimiser.step()
        rnd_loss = rnd_loss.detach()
        # normal_training
        update_metric = None
        for common_update_time in range(1):
            self.mac.agent.train()
            mac_out = []
            self.mac.init_hidden(batch.batch_size)
            for t in range(batch.max_seq_length):
                agent_outs = self.mac.forward(batch, t=t,update_metric=update_metric)
        ...
        update_metric = th.clamp(1*(rnd_loss-rnd_loss.mean(dim = 2).unsqueeze(2))/(rnd_loss.std(dim = 2).unsqueeze(2)+1e-8),0,3).to(th.int)
        # print("average extra training epoch:",update_metric.float().mean())
        # Calculate estimated Q-Values
        avg_traing_time = update_metric.float().mean()
        while update_metric.sum()>1e-8:
            update_metric[update_metric>0] -= 1
            self.mac.agent.train()
            mac_out = []
            self.mac.init_hidden(batch.batch_size)
            for t in range(batch.max_seq_length):
                agent_outs = self.mac.forward(batch, t=t,update_metric=update_metric[:, t, :])
                mac_out.append(agent_outs)
            mac_out = th.stack(mac_out, dim=1)  # Concat over time
        ...
        if t_env - self.log_stats_t >= self.args.learner_log_interval:
            ...
            # self.logger.log_stat("avg_extra_training_time", avg_traing_time.item(), t_env)
